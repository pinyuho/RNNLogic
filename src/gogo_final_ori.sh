...
for CLUSTER_SIZE in "${CLUSTER_SIZES[@]}"; do
  ...
  # sed -i "s|^save_path: .*|save_path: results/${DATASET}/multitask_mmoe/ent_all_types/k7_${EXP_NAME}_${STAMP}|" "$CONFIG_PATH"
  # sed -i "s|^save_path: .*|save_path: results/${DATASET}/sm_test/${STAMP}|" "$CONFIG_PATH" # FIXME:
  sed -i "s|^save_path: .*|save_path: results/${DATASET}/oris/ori1|" "$CONFIG_PATH" # FIXME:
