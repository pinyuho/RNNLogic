...
GPUS_PER_NODE=2
SUBSET_RATIO=1
PYTHON_SCRIPT="run_rnnlogic_ori.py"
...
for CLUSTER_SIZE in "${CLUSTER_SIZES[@]}"; do
  for ITER in {1..10}; do # FIXME:
      ...
      sed -i "s|^save_path: .*|save_path: results/${DATASET}/10_times_each/original_rnnlogic/${ITER}_${STAMP}|" "$CONFIG_PATH"
      # sed -i "s|^save_path: .*|save_path: results/${DATASET}/sm_test/${ITER}_${STAMP}|" "$CONFIG_PATH" # FIXME:
