...
def plot_elbow_curve(X, max_k=15, title="Elbow Method for Optimal k", save_path="kmeans_elbow.png"):
    """
...
def generate_relation_cluster_dict(triples, entityid_to_typeid, num_clusters, output_path="relation_cluster.dict"):
    ...
    # 3. 聚類
    relation_ids = list(relation_features.keys())
    X = torch.stack([relation_features[r] for r in relation_ids])
    plot_elbow_curve(X, max_k=12)
    ...
    with open(output_path, "w") as f:
        relid_to_cluster = {r: cid for r, cid in zip(relation_ids, cluster_labels)}
        for r in sorted(relid_to_cluster):
            f.write(f"{r}\t{relid_to_cluster[r]}\n")
...
def get_clustered_rels_report(relid2clusterid, id2rel, num_clusters, path="clustered_rels.txt"):
    res = dict()
    ...
    with open(path, "w", encoding="utf-8") as f:
        for cluster_id in sorted(res):
            ...
            for relation in sorted(res[cluster_id]):
                f.write(f"  - {relation}\n")
            f.write("\n")  # 每個 cluster 間空一行
    print(f"[✓] Cluster dictionary saved to {path}")
    

if __name__ == "__main__":
    # generate_lexname_dict("../../data/wn18rr/entities.dict")

    graph = KnowledgeGraph("../../data/semmed")
    # print(graph.train_facts)
    
    entityid_to_typeid = load_entity_type_dict("semmed/entity_type.dict")

    id2rel = {}
    with open("../../data/semmed/relations.dict", "r", encoding="utf-8") as f:
        for line in f:
    ...
    # Step 1
    num_clusters = 4
    # generate_relation_cluster_dict(graph.train_facts, entityid_to_typeid, num_clusters)

    # Step 2
    relid2clusterid = {}
    with open("semmed/relation_cluster.dict", "r", encoding="utf-8") as f:
        for line in f:
            relid, clusterid = line.strip().split("\t")
            relid2clusterid[int(relid)] = int(clusterid)
    get_clustered_rels_report(relid2clusterid, id2rel, num_clusters)
